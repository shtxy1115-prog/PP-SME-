...
(function startQuotationApp() {
  ...
  function applyWorksheetPrintXml(xml, sheet) {
    ...
    if (/<pageMargins\b[^>]*\/>/.test(styled)) {
      styled = styled.replace(/<pageMargins\b[^>]*\/>/, pageMargins);
    } else if (/<pageSetup\b[^>]*\/>/.test(styled)) {
      styled = styled.replace(/(<pageSetup\b[^>]*\/>)/, `${pageMargins}$1`);
    } else {
      styled = styled.replace(/<\/worksheet>/, `${pageMargins}${pageSetup}</worksheet>`);
      return styled;
    }
    if (/<pageSetup\b[^>]*\/>/.test(styled)) {
      styled = styled.replace(/<pageSetup\b[^>]*\/>/, pageSetup);
    } else {
      styled = styled.replace(/<\/worksheet>/, `${pageSetup}</worksheet>`);
    }
    return styled;
  }
