...
(function startQuotationApp() {
  ...
  function wrapDisplayLine(line, maxWidth) {
    const wrapped = [];
    let current = "";
    let currentWidth = 0;
    Array.from(line).forEach(character => {
      const characterWidth = displayCharacterWidth(character);
      while (current && currentWidth + characterWidth > maxWidth) {
        const breakAt = Math.max(current.lastIndexOf(" "), current.lastIndexOf("\u3000"));
        if (breakAt > 0) {
          // Keep the separator at the start of the next line. Excel for Mac
          // can treat a trailing space immediately before a manual line break
          // as ordinary whitespace inside merged cells and rejoin the lines.
          // Moving it preserves the original characters while making the
          // display break effective.
          wrapped.push(current.slice(0, breakAt));
          current = current.slice(breakAt);
          currentWidth = Array.from(current).reduce((sum, item) => sum + displayCharacterWidth(item), 0);
        } else {
  ...
  function wrapDisplayText(value, width) {
    const maxWidth = Math.max(10, width * TOB_TEXT_WRAP_FACTOR);
    return String(value ?? "").split("\n").map(line => wrapDisplayLine(line, maxWidth)).join("\n");
  }

  function prepareDisplaySheet(sheet) {
    if (!String(sheet?.name || "").includes("TOB")) return sheet;
    return {
      ...sheet,
      rows: (sheet.rows || []).map((row, rowIndex) => row.map((value, columnIndex) => {
        if (typeof value !== "string" || !value) return value;
        return wrapDisplayText(value, mergedColumnWidth(sheet, rowIndex, columnIndex));
      })),
    };
  }

  function rowHeight(row, sheet, rowIndex) {
    const isTob = String(sheet?.name || "").includes("TOB");
    const widthFactor = isTob ? TOB_TEXT_WRAP_FACTOR : 0.9;
    const lines = row.reduce((max, value, columnIndex) => {
      ...
      const lineCount = String(value ?? "").split("\n").reduce((count, line) => {
        ...
        return count + Math.max(1, Math.ceil(displayWidth / Math.max(10, width * widthFactor)));
      }, 0);
  ...
  const TOB_STYLE_IDS = Object.freeze({
    title: 1,
    header: 14,
    section: 15,
    metaLabel: 16,
  ...
  function styleIdFor(kind, isLabel, numberFormat, sheetName = "") {
    if (String(sheetName).includes("TOB")) {
      ...
      return isLabel ? TOB_STYLE_IDS.bodyLabel : TOB_STYLE_IDS.bodyValue;
    }
  ...
  function buildStylesXml() {
    ...
    const fonts = [
      ...
      font("OPPOSans R", 11, true, WORKBOOK_COLORS.blue),
      font("OPPOSans R", 11, true, WORKBOOK_COLORS.ink),
      font("OPPOSans R", 11, false, WORKBOOK_COLORS.ink),
    ].join("");
    ...
    const cellXfs = [
      ...
      xf(10, 3, 2, 0, "center", "center"),
      xf(10, 4, 4),
      xf(10, 5, 1),
      xf(12, 5, 1),
      xf(11, 8, 1),
      xf(12, 8, 1),
      xf(12, 8, 1, 165, "right", "top", true),
      xf(12, 8, 1, 164, "right", "top", true),
    ].join("");
    return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="2"><numFmt numFmtId="164" formatCode="${CURRENCY_FORMAT}"/><numFmt numFmtId="165" formatCode="${INTEGER_FORMAT}"/></numFmts><fonts count="13">${fonts}</fonts><fills count="9">${fills}</fills><borders count="6">${borders}</borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="${cellXfs.match(/<xf\b/g).length}">${cellXfs}</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles><dxfs count="0"/><tableStyles count="0" defaultTableStyle="TableStyleMedium9" defaultPivotStyle="PivotStyleMedium4"/></styleSheet>`;
  }
  ...
  function applyWorksheetPrintXml(xml, sheet) {
    ...
    const pageSetupProperties = `<sheetPr><pageSetUpPr fitToPage="1"/></sheetPr>`;
    let styled = xml;
    if (/<sheetPr\b[\s\S]*?<\/sheetPr>/.test(styled)) {
      styled = styled.replace(/<sheetPr\b[\s\S]*?<\/sheetPr>/, pageSetupProperties);
    } else {
      styled = styled.replace(/(<worksheet\b[^>]*>)/, `$1${pageSetupProperties}`);
    }
    if (/<sheetView\b[^>]*>/.test(styled)) {
      styled = styled.replace(/<sheetView\b([^>]*?)(\/?)>/, (opening, attributes, selfClosing) => {
        const cleanedAttributes = attributes.replace(/\s+showGridLines="[^"]*"/g, "");
        return `<sheetView showGridLines="0"${cleanedAttributes}${selfClosing}>`;
      });
    } else {
      const sheetViews = `<sheetViews><sheetView showGridLines="0" workbookViewId="0"/></sheetViews>`;
      styled = /<dimension\b[^>]*\/>/.test(styled)
        ? styled.replace(/(<dimension\b[^>]*\/>)/, `$1${sheetViews}`)
        : styled.replace(/(<sheetPr\b[\s\S]*?<\/sheetPr>)/, `$1${sheetViews}`);
    }
    // Worksheet children follow a schema order. In particular, pageMargins/pageSetup
    // must be before ignoredErrors; appending them after ignoredErrors makes Excel
    // enter recovery mode and may discard the visible worksheet content.
    styled = styled.replace(/<pageMargins\b[^>]*\/>/g, "").replace(/<pageSetup\b[^>]*\/>/g, "");
    const pageBlock = `${pageMargins}${pageSetup}`;
    const tailAnchor = /<(?:headerFooter|rowBreaks|colBreaks|customProperties|cellWatches|ignoredErrors|smartTags|drawing|legacyDrawing|legacyDrawingHF|picture|oleObjects|controls|webPublishItems|tableParts|extLst)\b/;
    if (tailAnchor.test(styled)) styled = styled.replace(tailAnchor, `${pageBlock}$&`);
    else styled = styled.replace(/<\/worksheet>/, `${pageBlock}</worksheet>`);
    return styled;
  }
  ...
  function styleWorksheetXml(xml, sheet) {
    ...
    const styled = xml.replace(/<c\b([^>]*?)(\/?)>/g, (opening, attributes, selfClosing) => {
      ...
      const styleId = styleIdFor(kind, styleColumnIndex === 0, numberFormat, sheet.name);
    ...
    return applyWorksheetPrintXml(styled, sheet);
  }
  ...
  async function exportExcel() {
    ...
    const displayModel = { ...model, sheets: model.sheets.map(prepareDisplaySheet) };
    ...
    let bytes;
    try {
      bytes = await styleWorkbookBytes(rawBytes, displayModel);
    } catch (error) {
