...
|---|---|---|---|
| 计划与区域 | Plan 3、Plan 4 都要支持 WWE 与 WW；WWE=全球除美，WW=全球 | 只有 `P3WWE`、`P4WW`、`P4WWE`；缺 `P3WW`，且 `P4WWE` 区域文案错误 | P0 |
| 费率 | `Quotation` 工作表给出 P1O1-P1O3、P2O1-P2O3、P3WWE、P4WW、P4WWE 九列；65-69 有真实数字，70-75 为个案核保 | 65+ 被校验拦截；`rateFor(...) || 0` 把无费率变成 0 保费 | P0 |
| P3WW 费率 | 真实费率表未发现 P3WW 列 | 当前也未建 P3WW；不能把 P4WW 或其他列当作替代 | 需保留为 `NEEDS_CONFIRMATION`，进入人工费率边界 |
...
1. `PLANS` 不是从费率表和方案表完整建模：缺 `P3WW`，并且计划区域/费率列映射不完整。
2. 年龄与费率路径把“没有自动费率”“待人工核保”“不符合条件”混成数字 0 或硬错误，导致 65-69 被错误阻断，也会静默低报 70+ / P3WW。
...
- 重建计划/区域/费率列元数据，并明确 P3WW 为缺少原始费率列的 `NEEDS_CONFIRMATION` / 人工费率路径。
...
- `Quotation` 原始费率表没有 `P3WW` 费率列。P3WW 可以出现在计划模型中，但在费率得到确认前只能走 `PENDING_UW` / 人工费率路径；不能复用 `P3WWE`、`P4WW` 或 `P4WWE`。
...
缺少 P3WW 原始费率列是本轮唯一明确的业务确认边界，不阻止其余可靠性修复。下一步直接建立失败回归并在上述精确范围内实现 v4；最终以回归结果、浏览器导出和解析后的 XLSX 结构证据决定是否仍有 P0。
