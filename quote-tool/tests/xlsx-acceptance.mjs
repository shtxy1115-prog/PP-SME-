...
const fixedCases = [
  ...
  },
  {
    ...
    check() {
      ...
      assert.equal(quotation.rows[1][0], "文件版本 File Version");
      assert.equal(quotation.rows[1][1], "");
      assert.match(quotation.rows[1][4], /Core Reliability v4/);
      assert.ok(quotation.merges.includes("A2:D2"));
      assert.ok(quotation.merges.includes("E2:G2"));
  ...
  },
  {
    ...
    check() {
      ...
      assert.match(preExistingRow[4], /最高等级 FMU/);
      assert.equal(discountRow[4], 19905);
      assert.equal(totalRow[4], 122268);
      assert.equal(Number.isInteger(discountRow[4]), true);
      assert.equal(Number.isInteger(totalRow[4]), true);
