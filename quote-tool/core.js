...
(function attachQuoteCore(root, factory) {
  ...
  function buildTobBlock(rows, rowStyles, merges, variant, index, state, title = false) {
    ...
    BENEFIT_DATA.forEach(item => {
      ...
      rows.push([benefitDisplayName(item), "", tobCoverage(item, variant), "", ""]);
      rowStyles.push("body");
  ...
  function buildParallelTobSheet(state, variants) {
    ...
    BENEFIT_DATA.forEach(item => {
      if (item.section !== previousSection) {
        const sectionRow = rows.length + 1;
        rows.push([SOURCE_SECTIONS[item.section], ...blanks(columnCount - 1)]);
        rowStyles.push("section");
      ...
      const benefitRow = rows.length + 1;
      rows.push([
        benefitDisplayName(item),
        "",
        ...variants.flatMap(variant => [tobCoverage(item, variant), ""]),
      ]);
      rowStyles.push("body");
