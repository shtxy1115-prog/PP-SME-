...
(function attachQuoteCore(root, factory) {
  ...
  function buildQuotationSheet(state, variants) {
    const summaryColumnCount = Math.max(4, 4 + variants.length * 3);
    const summaryRow = (label, values = []) => {
      const row = Array(summaryColumnCount).fill("");
      row[0] = label;
      values.forEach((value, index) => {
        const valueIndex = 4 + index * 3;
        if (valueIndex < row.length) row[valueIndex] = value;
      });
      return row;
    };
    const rows = [
      ["Prosper × Marsh SME 团体医疗保险报价表 Group Medical Insurance Quotation"],
      summaryRow("文件版本 File Version", ["Core Reliability v4 · Source: " + SOURCE_WORKBOOK]),
      summaryRow("团体中文名称 Company Name (Chinese)", [state.companyCn || ""]),
      summaryRow("团体英文名称 Company Name (English)", [state.companyEn || ""]),
      summaryRow("保障期限 Policy Period", [`${state.startDate || ""} 至 / to ${state.endDate || ""}`]),
      summaryRow("支付条件 Payment Condition", [state.pcpDirectBilling ? "需通过柏盛 PCP 首诊方可涵盖直付服务（急诊除外）；医疗保费下调3%\nDirect billing is covered only through a 柏盛 PCP first visit; emergencies are excluded; 3% Medical premium discount" : "未选择柏盛 PCP 首诊直付服务\n柏盛 PCP direct billing not selected"]),
      summaryRow("自付比例 Policy Co-payment", variants.map(variant => selectedCopay(variant).label)),
      summaryRow("既往症安排 Pre-existing Conditions", variants.map(variant => selectedPreExisting(variant).label)),
      summaryRow("报价模式 Quotation Mode", [state.mode === "compare" ? "同一批人员多方案比价 / Compare" : "按人员分配不同方案 / Group"]),
      summaryRow("保费汇总 Premium Summary", variants.map(variantLabel)),
      summaryRow("参保人数 Insured Members", variants.map(variant => `${membersForVariant(state, variant).length} 人 / members`)),
      summaryRow("基础医疗保费 Base Medical Premium", variants.map(variant => totalsForVariant(state, variant).baseMedical)),
      summaryRow("医疗保费优惠 Medical Discount", variants.map(variant => totalsForVariant(state, variant).discount)),
      summaryRow("可选福利保费 Optional Benefits Premium", variants.map(variant => totalsForVariant(state, variant).optional)),
      summaryRow("最终保费 Total Premium", variants.map(variant => totalsForVariant(state, variant).total)),
      summaryRow("人员保费明细 Member Premium Details", variants.map(variant => `${variant.planCode} · ${variant.name || "未命名方案"}`)),
    ...
    const widths = [10, 28, 18, 10, ...variants.flatMap(() => [29, 22, 24])];
    const lastColumn = columnName(summaryColumnCount - 1);
    const merges = [`A1:${lastColumn}1`];
    for (let rowNumber = 2; rowNumber <= 16; rowNumber += 1) {
      merges.push(`A${rowNumber}:D${rowNumber}`);
      variants.forEach((_, index) => {
        const start = columnName(4 + index * 3);
        const end = columnName(6 + index * 3);
  ...
  function buildPremiumSheet(state, variants) {
    const totalColumns = Math.max(2, variants.length + 1);
    const sourceRow = Array(totalColumns).fill("");
    sourceRow[0] = "来源 Source";
    sourceRow[1] = `${SOURCE_WORKBOOK}\n费率工作表 / Rate Sheet：${SOURCE_SHEET_RATES}`;
    const rows = [
      ["费率表 Premium"],
      sourceRow,
    ...
    rowStyles.push("body");
    const widths = [24, ...variants.map(() => variants.length === 1 ? 42 : 34)];
    if (variants.length === 0) widths.push(42);
    const lastColumn = columnName(totalColumns - 1);
    const merges = [`A1:${lastColumn}1`];
    if (totalColumns > 2) merges.push(`B2:${lastColumn}2`);
    return { name: "费率 Premium", rows, rowStyles, widths, merges };
  }
